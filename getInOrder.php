...
<?php
  ...
  session_start();
  $uid=$_SESSION['id'];
  $sql= " SELECT c.name,c.price,s.quantity FROM shoppingbag as s,commodity as c WHERE uid='$uid' AND cid=comid AND finished=0";
...
<div id="pricing" class="container-fluid">
  ...
  <div class="row"> 
	<form action="" method="post">
		<table border="1" width="80%" style="color:#000000">
		<tr><td>商品名稱</td><td>價格</td><td>購買數量</td></tr>
		<?php
			$total_price=0;
			$sql= " SELECT c.name,c.price,s.quantity,c.quantity FROM shoppingbag as s,commodity as c WHERE uid='$uid' AND cid=comid AND finished=0";
			$result=$conn->query($sql);
		...
		?>
		<?php $col_total_price=$r[1]*$r[2];
			$total_price+=$col_total_price;
			
		?>
		<tr><td><?php echo $r[0] ?></td><td><?php echo $r[1] ?></td><td><input type='number' name='quantity.+<?php echo $i ?>' min='0' max='<?php echo $r[3]?>' value='<?php echo $r[2]?>'> 還剩餘<?php echo $r[3]?>件</td></tr>
    
		<?php
			$i=$i+1;
			}
			$sql= "SELECT a.amount FROM account a,user u WHERE id='$uid' and a.aid=u.aid";
			$result=$conn->query($sql);
			$r=$result->fetch_row();
		?> 
	<tr><td align="right" colspan='3'><h2>總計： <?php echo $total_price ?><br>帳戶餘額：<?php echo $r[0] ?><h2></td></tr>
    </table>
	<div class="panel-footer">
	<input type="button" name="button" value="回到購物車" onclick="location.href='shoppingbag.php'" style="color:#000000;background:#FFB7DD">
	<input type="button" name="button" value="完成結帳" onclick="location.href='confirmCheck.php?value='+<?php echo $total_price ?>" style="color:#000000;background:#FFB7DD">
	<!-- <input type='submit' name='button' value='完成結帳' onclick='updateCart()' style="color:#FFB7DD"> -->
	</form>
	</div>
  </div>
</div>
