...
<div style="text-align:center;margin:0px auto;">
 ...
 <input type="password" name="pw" required="TRUE" placeholder="密碼"/><br/>
 <input type="text" style="width: 50%;padding: 12px 20px;margin: 8px 0;display: inline-block;border: 1px solid #ccc;border-radius: 4px;box-sizing: border-box;" name="name" required="TRUE" placeholder="姓名"/><br/>
  <input type="text" style="width: 50%;padding: 12px 20px;margin: 8px 0;display: inline-block;border: 1px solid #ccc;border-radius: 4px;box-sizing: border-box;" name="account" required="TRUE" placeholder="銀行帳號"/><br/>
  <input type="number" style="width: 50%;padding: 12px 20px;margin: 8px 0;display: inline-block;border: 1px solid #ccc;border-radius: 4px;box-sizing: border-box;" name="money" required="TRUE" placeholder="銀行帳號存款"/><br/>
...
<?php
  ...
  $money=$_POST['money'];
	...
	if(isset($username)&&isset($pw)&&isset($gender)&&isset($name)){
		if (!preg_match("/([\w\-]+\@[\w\-]+\.[\w\-]+)/","$username")) {
			echo "電子信箱格式錯誤!";
			echo "<script>setTimeout(function(){window.location.href='register.php';},1500);</script>";
		}else{
      $sql="INSERT INTO account(aid,amount) VALUES ('$account','$money')";
      $reslut = $conn->query($sql);
			$sql="INSERT INTO user(Id,Name,Gender,Username,Password,aid) VALUES (null,'$name','$gender','$username','$pw','$account')";//向資料庫插入表單傳來的值的sql
			$reslut = $conn->query($sql);//執行sql
			if (!$reslut){
				echo "此帳號已註冊";
				echo "<script>setTimeout(function(){window.location.href='login.php';},1500);</script>";
			}else{
        $_SESSION['regist_time']=date("Y-m-d");
				header("refresh:1;URL=home.php"); //自動跳轉回主頁面
			}
		}
	}else{
