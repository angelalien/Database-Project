<?php
  session_start();
  $uid=$_SESSION['id'];
  $total_price=$_GET['value'];
  //echo $total_price;
  include("connMySQL.php"); 
  $sql= " SELECT a.amount FROM account a,user u WHERE a.aid=u.aid and u.id='$uid'";
...
<div id="pricing" class="container-fluid">
  <div class="text-center">
	<?php
		if($r[0]<$total_price){
	?>
			<h1><?php echo "結帳失敗，帳戶餘額不足";?></h1>
	<?php		
		}
		else {
			$balance=$r[0]-$total_price;
			$sql="UPDATE account a,user u SET amount=$balance WHERE u.id='$uid' and u.aid=a.aid";
			if($conn->query($sql) === FALSE){
	?>
		<h1><?php echo "結帳未完成";?></h1>
	<?php	}
			else{
	?>
		<h1><?php echo "結帳完成";?></h1>			
	<?php
				$sql= " SELECT a.amount,a.aid FROM account a,user u,shoppingbag s,commodity c WHERE a.aid=u.aid and s.cid=c.comid and s.uid='$uid' and u.id=c.userid and s.finished=0";
				$result=$conn->query($sql);
				while($r=$result->fetch_row()){
					$balance=$r[0]+$total_price;
					$sql="UPDATE account a SET amount=$balance WHERE aid=$r[1]";
					$conn->query($sql);
					
				}
				$sql="SELECT c.comid,c.quantity FROM commodity c,shoppingbag s WHERE s.uid='$uid' and c.comid=s.cid and s.finished=0";
				$result=$conn->query($sql);
				$sql2="SELECT cid,quantity FROM shoppingbag WHERE uid='$uid' and finished=0";
				$result2=$conn->query($sql2);
				while($r=$result->fetch_row()){
					$r2=$result2->fetch_row();
					if($r[0]==$r2[0] and $r[1]>=$r2[1]){
						$quantity=$r[1]-$r2[1];
						$sql3="UPDATE commodity SET quantity=$quantity WHERE comid=$r[0]";
						$conn->query($sql3);
					}
				}
				$sql="UPDATE account a,user u SET amount=$balance WHERE u.id= and u.aid=a.aid";
				$sql ="UPDATE shoppingBag SET finished=1, time=now() WHERE uid='$uid' and finished=0";
				$result=$conn->query($sql);
			}
  ...
  <div class="row"> 
	...
	<center><input type="button" name="button" value="查看訂單" onclick="location.href='orderlist.php'" style="color:#000000;background:#FFB7DD">                <input type="button" name="button" value="回首頁" onclick="location.href='home.php'" style="color:#000000;background:#FFB7DD"></center>
	</div>
  </div>
</div>
